...
//        System.out.println(findPeak(ints));
 //       double[] wave = {563.00, 563.00, 563.00, 564.00, 564.00, 564.00, 564.00, 564.00, 564.00, 564.00, 564.00, 564.00, 564.00, 564.00, 564.00, 564.00, 564.00, 565.00, 565.00, 565.00, 565.00, 565.00, 565.00, 565.00, 565.00, 565.00, 565.00, 565.00, 565.00, 565.00, 565.00, 565.00, 565.00, 566.00, 565.00, 566.00, 566.00, 566.00, 566.00, 566.00, 566.00, 566.00, 566.00, 566.00, 566.00, 566.00, 566.00, 566.00, 566.00, 567.00, 567.00, 567.00, 567.00, 567.00, 567.00, 567.00, 567.00, 567.00, 567.00, 567.00};
        ...
        //double[] wave = { -6,-4,-3,2,2,2,2};
  //      double[] wave ={  553.0, 606.0, 657.0, 698.0, 758.0, 816.0, 879.0, 904.0, 904.0, 906.0, 910.0, 910.0, 910.0, 910.0, 910.0, 910.0, 910.0, 910.0, 910.0, 910.0, 910.0, 910, 910.0} ;


  //     double[] wave = { 553.1600000000001, 604.4000000000001, 655.64, 705.6560000000002, 753.056, 795.9040000000001, 832.3520000000001, 861.1600000000001, 882.0, 895.472, 903.2639999999999, 907.256, 909.0, 909.616, 909.856, 909.9680000000001, 910.0, 909.0, 910.0, 910.0, 910.0, 910.0, 910.0};


  //    double[] wave ={ 300,250,240,230,220,200,200,200,200,200,250,260 ,260,260,260};

       double[] wave ={10,20,30,40,50,40,30,60,60,60,60,60};

   //     double[] wave ={40,30,10,10,10,10,10,10};
     //      double[] wave ={291.0, 291.289, 291.578, 291.874, 292.18399999999997, 292.508, 292.85600000000005, 293.216, 293.58399999999995, 293.952, 294.312, 294.656, 295.0, 295.336, 295.664, 296.0, 296.336, 296.65999999999997, 296.98400000000004, 297.30199999999996, 297.606, 297.907, 298.20799999999997,298.20799999999997,298.20799999999997};
     //           System.out.println(wave.length);

      //  double[] wave ={10,10,10,10,10,10};

        //      byte[] wave = new byte[]{4,5,1,1,4,3,3,3,3,5,5,6,4,4,3,2};
        int direction = wave[0] > 0 ? -1 : 1;

        for (int i = 1; i < wave.length - 2; i++) {


            if ((wave[i + 1] - wave[i]) * direction > 0  && (wave[i] - wave[i-1])*direction <=0) {
                direction *= -1;
                if (direction == 1) {
    ...
    public static  void  findTrade(double[] arrayList){
        // 计算一阶差分
        double[] diff1=new double[arrayList.length-1];
        for(int i=0 ; i<arrayList.length-1 ; i ++){
            diff1[i]=arrayList[i+1]-arrayList[i];
        }

        // 取符号运算
        double[] trend1=new double[diff1.length];
        for(int j=0 ; j < diff1.length ; j++){
            if(diff1[j] > 0){
                trend1[j]=1;
            }else if (diff1[j] <0){
                trend1[j]=-1;
        ...
        // 从尾部遍历向量tread1,处理为0的情况
        int length = trend1.length-1;
        for(int n=0 ; n< length ; n++){
            if(trend1[length-n-1]==0 && trend1[length-n] >=0){
                trend1[length-n-1]=1;
            }
            if(trend1[length-n-1]==0 && trend1[length-n]<0){
                trend1[length-n-1]=-1;
        ...
        // 再次计算一阶差分
        double[] diff2=new double[trend1.length-1];
        for(int m=0 ; m<trend1.length-1 ;m++){
            diff2[m]=trend1[m+1]-trend1[m];
        }
        ...
        for (int k=0 ; k < diff2.length ; k++){
            if(diff2[k]==-2){
                System.out.println("波峰" +(k+1)+";"+arrayList[k+1]);
                temp=arrayList[k+1];
            }
            if(diff2[k]==2){
                System.out.println("波谷"+(k+1)+";"+arrayList[k+1]);
                temp=arrayList[k+1];
            }
        ...
        System.out.println("最后一位：" + diff2[diff2.length-1]);

        if(-1 == diff2[diff2.length-1] && Objects.nonNull(temp) && temp != arrayList[arrayList.length-1]){
            System.out.println("波峰A"+arrayList[arrayList.length-1]);
        }
